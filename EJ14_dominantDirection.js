/*
Eloquent Javascript, page 96, make a function to check the most common writing direction
of a string of text, by checking the characters against a big ol' object made by
...
//I'll just include the whole scripts array up here -- scroll down for the functions etc.
...
//THE FUNCTION I AM WRITING:
function dominantDirection(text) {
    if (text.length < 1) return undefined; //in case it's empty
    
    let charactersArray = [];
    for (let character of text) { //putting characters in array to avoid UTF-16 problems
        charactersArray = charactersArray.concat(character);
    }

    let directionsCount = countBy(charactersArray, character => { //get count of each direction
        if (characterScript(character.codePointAt(0)) !== null) {
            return characterScript(character.codePointAt(0)).direction;
        }
    });
    console.log(directionsCount)
    
    let isTied = false; //used to return undefined later if the count was a tie;
    let highestCount = directionsCount.reduce( (highestCount, currentCount) => {
        if (currentCount.count > highestCount.count) {
            isTied = false;
            return currentCount;
        }
        else if (currentCount.count == highestCount.count) {
            isTied = true;
            return currentCount;
        }
        else return highestCount;
    } );
    
    if (isTied) return undefined;
    else return highestCount.name;
}
...
console.log(dominantDirection("Hey, خير")); //undefined
console.log(dominantDirection("")); //undefined
